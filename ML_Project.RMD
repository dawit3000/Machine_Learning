\-\-- title: "ML Project" author: "Dawit Aberra" date: August 2022 output: 
  html_document:
    keep_md: true \-\--
...
Remove columns irrelevant to the outcome Inspecting the variables verses the outcome suggests that the firest 7 variables are fairly irrelevant to the expected outcome. We decided to remove these variables.
...
Remove variables with zero and near-zero variance. Datasets come sometimes with predictors that take a unique value across sample. Such uninformative predictor is more common. This kind of predictor is not only non-informative, it can break some models [1]. We decided to remove variables with near zero variance.
